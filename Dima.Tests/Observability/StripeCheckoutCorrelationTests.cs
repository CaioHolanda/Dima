...
public sealed class StripeCheckoutCorrelationTests
{
    ...
    public async Task RetriedAttemptPreservesStripeParametersAndConnectsHttpLogs(bool legacy)
    {
        ...
        var previousKey = Dima.Api.ApiConfiguration.StripeApiKey;
        StripeConfiguration.StripeClient = new StripeClient("sk_test_dt19", httpClient:
            new SystemNetHttpClient(http, maxNetworkRetries: 0, enableTelemetry: false));
        Dima.Api.ApiConfiguration.StripeApiKey = "sk_test_dt19";
        ...
        try
        {
            foreach (var id in requestIds)
            {
                ...
                db.Orders.Add(new Order
                {
                    ...
                    Status = EOrderStatus.WaintingPayment, PaymentSessionExpiresAt = expiration,
                ...
                var result = await new StripePaymentHandler(db, TimeProvider.System,
                    Options.Create(new OrderExpirationOptions()), logger).CreateSessionAsync(new CreatePaymentSessionRequest
        ...
        finally
        {
            ...
            Dima.Api.ApiConfiguration.StripeApiKey = previousKey;
        }
    }
